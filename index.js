...
(async () => {
  // todo: this shouldn't reset ghost's followers/following
  const ghostUser = {
  ...
  var findGhostUser = await users.findOne({
    _id: new ObjectId("000000000000000000000000"),
  });
  if (findGhostUser) {
...
app.post(
  ...
  checkLoggedOut((req, res) =>
    res
      .status(412)
      .json({ error: "can't create new account whilst logged in" }),
  ) /* ditto */,
  rateLimit({
    windowMs: 3600000,
    max: 5,
...
app.get("/api/top/users", async (req, res) => {
  ...
  var top = await topCursor.toArray(); // <-- convert cursor to array

  res.json(
    top.map((u) => ({ id: u._id, name: u.name, followers: u.followers })),
  );
});
...
app.get("/api/trending/posts", async (req, res) => {
  const trending = await posts.aggregate([{ $sample: { size: 10 } }]);

  for (let post of trending) {
    const poster = await findUserDataByID(post.poster);
    post.poster = {};
    if (poster) {
      post.poster.name = poster.name;
      post.poster.id = poster._id;
    } else {
      post.poster.name = "ghost";
      post.poster.id = "000000000000000000000000";
    }
  }
...
app.get("/api/following/posts", checkLoggedIn(), async (req, res) => {
  ...
  var following = await users.find({
    followers: { $all: [user._id] },
  });
  var postsByFollowing = await posts.find(
    { poster: { $in: following.map((f) => f._id) } },
...
app.get("/api/messages/count", checkLoggedIn(), async (req, res) => {
  var user = res.locals.requester;
  var msgs = await messages.find({ to: user._id, read: false });
...
app.get("/api/users/:user/following", async function (req, res, next) {
  var user = await findUserData(req.params.user);
  var followingDB = await users.find({
    followers: { $all: [user._id] },
  });
  var following = [];
...
app.delete("/picture/:user", checkLoggedIn(), async (req, res) => {
  ...
  if (!req.xhr)
    return res.status(403).json({ error: "must be requested with xhr" });
    ...
    try {
      ...
      if (
        ...
        await fs.promises.writeFile(
          `./uploads/profiles/${req.params.user}.png`,
          binaryData,
          "binary",
        );
...
app.get("/picture/:user", async function (req, res, next) {
  ...
  if (user) {
    if (fs.existsSync(`./uploads/profiles/${user._id}.png`)) {
      // if the user has an image, send that
      res.sendFile(__dirname + `/uploads/profiles/${user._id}.png`);
    } else {
      // if the user exists but doesnt have an image make one from their name
      var file = jdenticon.toPng(user.name, 128);
      res.set("Content-Type", "image/png");
      res.send(file);
    }
  } else {
    // if the user doesn't exist, resort to using the id from url to make the image
        ...
        if (content) {
          comments
            ...
            .then(async (comment) => {
              await addMessage(
                post.poster,
                `<a href='/users/${user.name}'>@${user.name}</a> commented on <a href='/posts/${post._id}'>your post</a>.`,
              );
...
app.delete("/posts/:post", checkLoggedIn(), async function (req, res, next) {
  ...
  if (req.xhr) {
    if (post) {
      ...
      if (user.admin || post.poster.toString() == user._id) {
        try {
          await posts.remove({ _id: post._id });
          await comments.remove({ post: post._id });
        ...
        res.status(403).json({
          error:
            "requested post for deletion is not made by the same user who requseted",
        });
      }
    } else {
      res.status(404).json({ error: "no post found" });
...
app.post("/posts/:id/love", checkLoggedIn(), async function (req, res) {
  ...
  if (req.xhr) {
        ...
        $set: {
          loves: {
            $cond: [
              ...
              {
                $setDifference: ["$loves", [user._id]],
              },
              {
                $concatArrays: ["$loves", [user._id]],
              },
            ],
          },
        },
    ...
    if (postDB) {
      if (postDB.loves.includes(user._id)) {
...
app.post("/users/:name/follow", checkLoggedIn(), async function (req, res) {
  ...
  if (req.xhr) {
        ...
        $set: {
          followers: {
            $cond: [
              ...
              {
                $setDifference: ["$followers", [user._id]],
              },
              {
                $concatArrays: ["$followers", [user._id]],
              },
            ],
          },
        },
    ...
    if (userDB) {
      var following = await users.find({
        followers: { $all: [userDB._id] },
      });
      if (userDB.followers.includes(user._id)) {
        res.json({
          ok: "now following",
          action: "follow",
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (user) {
        ...
        var update = await messages.insertOne(message);
